...
void AACodonMutSelFinitePhyloProcess::ReadMapStats(string name, int burnin, int every, int until){
	...
	// 	osmap << "\t" << "type";
		...
		// }
		// for(int k = 0; k < GetStateSpace()->GetNstate(); k++){
		// 	std::string s = GetStateSpace()->GetState(k);
		// 	if (s  == "TCG" ||  s  == "CCG" ||  s  == "CCG" ||  s  == "ACG" ||  s  == "GCG" ||  s  == "TTA" ||  s  == "CTA" ||  s  == "GTA"){
		// 		osmap << "\t" << s;
		// 	}
		...
		// for(int k = 0; k < GetStateSpace()->GetNstate(); k++){
		// 	for(int l = 0; l < GetStateSpace()->GetNstate(); l++){
		// 		std::string s = GetStateSpace()->GetState(k) + ">" + GetStateSpace()->GetState(l);
		// 		if (k != l ){
		// 			if (s  == "TCG>TCA" ||  s  == "CCG>CCA" ||  s  == "ACG>ACA" ||  s  == "GCG>GCA" ||  s  == "TTA>TTG" ||  s  == "CTA>CTG" ||  s  == "GTA>GTG") {
		// 				osmap << "\t" << s;
		// 			}
		// 		}
		// 	}
